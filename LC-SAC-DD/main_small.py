...
def run(args):
    ...
    if recurrent:
        latent_encoder = RecurrentLatentEncoderSmall(input_dim=latent_encoder_input_dim, latent_dim=5,
    ...
    policy = SquashedGaussianMLPActor(obs_dim + latent_dim, act_dim, hidden_sizes, activation, act_limit).to(args.device)
    ...
    algorithm = SAC(env, args.env, agent, q1_net, q2_net,
                    ...
                    gamma=args.gamma, seed=args.seed, epochs=args.epochs, max_ep_len=args.max_ep_len,seq_len=args.seq_len,
                    save_freq=args.save_freq,model_path=args.model_path,device=args.device,train_steps=args.train_steps,collect_data_samples=args.collect_data_samples,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_ep_len', type=int, default=1000)  # 默认是1000
    ...
    parser.add_argument('--collect_data_samples',type=int,default=5000)
