...
class VecTFPreferenceReward(VecEnvWrapper):
    def __init__(self, venv, num_models, model_dir, include_action, num_layers, embedding_dims, ctrl_coeff=0.,
                 alive_bonus=0.):
        VecEnvWrapper.__init__(self, venv)
        ...
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                from preference_learning_1 import Model  # revised by puyuan
                ...
                for i in range(num_models):
                    with tf.variable_scope('model_%d' % i):
                        model = Model(include_action, self.venv.observation_space.shape[0],
                                      self.venv.action_space.shape[0], num_layers=num_layers,
        ...
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                for model in self.models:
                    ...
                    # baselines/run.py  def build_env(args):
...
class VecTFPreferenceRewardNormalized(VecTFPreferenceReward):
    def __init__(self, venv, num_models, model_dir, include_action, num_layers, embedding_dims, ctrl_coeff=0.,
                 alive_bonus=0.):
        super().__init__(venv, num_models, model_dir, include_action, num_layers, embedding_dims, ctrl_coeff,
                         alive_bonus)
