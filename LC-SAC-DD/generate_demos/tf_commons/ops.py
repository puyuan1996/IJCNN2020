...
class WeightNormSymPadConv2d(object):  # Resize and Convolution(upsacle by 2)
    def __init__(self, name, input_dim, output_dim,
        ...
        assert k_h % 2 == 1 and k_w % 2 == 1, 'kernel size should be odd numbers to ensure exact size'
        with tf.variable_scope(name):
            self.conv2d = WeightNormConv2d('conv', input_dim, output_dim, k_h, k_w, 1, 1, data_format='NHWC',
                                           padding='VALID')
        self.padding = [[0, 0], [k_h // 2, k_h // 2], [k_w // 2, k_w // 2], [0, 0]]
...
class TransposedConv2d(object):
    ...
    def __call__(self, input_var, name=None, **xargs):
        ...
        if (self.data_format == 'NCHW'):
            shapes = tf.stack(
                [shapes[0], tf.shape(self.b)[0], shapes[2] * self.strides[2], shapes[3] * self.strides[3]])
        else:
            shapes = tf.stack(
                [shapes[0], shapes[1] * self.strides[1], shapes[2] * self.strides[2], tf.shape(self.b)[0]])
    ...
    def __init__(self, name, dims, axis=1, epsilon=1e-3, momentum=0.999, center=True, scale=True):
        ...
        with tf.variable_scope(name) as scope:
            with tf.variable_scope('bn'):
                ...
                self.moving_mean = tf.get_variable('moving_mean', [dims], initializer=tf.constant_initializer(0.0),
                                                   trainable=False)
                self.moving_variance = tf.get_variable('moving_variance', [dims],
