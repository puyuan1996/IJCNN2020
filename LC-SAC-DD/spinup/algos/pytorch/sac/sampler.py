...
def rollout(env, agent, max_path_length=np.inf, deterministic=False, render=False):
    observations = []
    actions = []
    rewards = []
    terminals = []
...
def obtain_rollout_samples(env, agent, max_path_length, max_samples, max_nums_paths=np.inf, deterministic=False ):
    """
    ...
    # policy = MakeDeterministic(self.policy) if deterministic else policy
